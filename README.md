...
[`.github/workflows/ci.yml`](.github/workflows/ci.yml) calls **`thadiust/workflow-python/.github/workflows/ci.yml@v1.0.2`** (bump the tag when you adopt a new release, or use **`@main`** for floating latest) with explicit **`ruff_version`** and **`pytest_version: "9.0.2"`** (keep both aligned with `requirements.txt`) and **`run_pytest: true`**. **`requirements.txt`** pins **`pytest==9.0.2`** so local installs match CI.
