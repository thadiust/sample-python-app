...
[`.github/workflows/ci.yml`](.github/workflows/ci.yml) calls **`thadiust/workflow-python/.github/workflows/ci.yml@v1.0.1`** (bump the tag when you adopt a new release, or use **`@main`** for floating latest) with explicit **`ruff_version`** (keep aligned with `requirements.txt`) and **`run_pytest: true`**. **`requirements.txt`** pins **`pytest==…`** to match workflow defaults.
